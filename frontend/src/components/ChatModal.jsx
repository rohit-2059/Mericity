...
const ChatModal = ({ isOpen, onClose, complaint }) => {
    ...
    if (isOpen && complaint) {
      const initializeChat = async () => {
        ...
        try {
          ...
          const chatResponse = await fetch('http://localhost:5000/chat/create', {
            method: 'POST',
            headers: {
  ...
  const loadMessages = async (chatId) => {
    try {
      ...
      await fetch(`http://localhost:5000/chat/read/${chatId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      // Surface minimal toast/alert once
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-semibold text-gray-800">
              Chat with {chat?.departmentId?.name || 'Department'}
            </h2>
            <p className="text-sm text-gray-600">
              Complaint: {complaint?.description?.slice(0, 50)}...
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            messages.map((message) => (
              ...
              >
                <div className="max-w-xs lg:max-w-md">
                  {/* Sender name */}
                  <div className={`text-xs text-gray-500 mb-1 ${isUserMessage(message) ? 'text-right' : 'text-left'}`}>
                    {isUserMessage(message) ? 'You' : (message.senderId?.name || 'Department')}
                  </div>
                  ...
                  {/* Timestamp */}
                  <div className={`text-xs text-gray-400 mt-1 ${isUserMessage(message) ? 'text-right' : 'text-left'}`}>
