    ...
    const fetchJanawaazData = async () => {
      try {
        ...
        const [districtRes, stateRes, pointsRes, statsRes, analyticsRes] = await Promise.all([
          fetch('http://localhost:5000/janawaaz/district', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          fetch('http://localhost:5000/janawaaz/state', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          fetch('http://localhost:5000/janawaaz/my-points', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          fetch('http://localhost:5000/janawaaz/stats', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          fetch('http://localhost:5000/janawaaz/analytics', {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
