...
const DepartmentChatModal = ({ isOpen, onClose, chat }) => {
    ...
    if (isOpen && chat) {
      const loadMessages = async () => {
        ...
        try {
          ...
          const response = await fetch(`http://localhost:5000/chat/messages/${chat._id}`, {
            headers: {
          ...
          await fetch(`http://localhost:5000/chat/read/${chat._id}`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`
            }
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        formData.append('senderType', 'department');
        formData.append('image', selectedImage);

        response = await fetch('http://localhost:5000/chat/message/image', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        // Send text message
        response = await fetch('http://localhost:5000/chat/message', {
          method: 'POST',
          headers: {
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Complaint: {chat?.complaintId?.description?.slice(0, 50)}...
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              >
                <div className="max-w-xs lg:max-w-md">
                  ...
                  <div className={`text-xs text-gray-500 mb-1 ${isDepartmentMessage(message) ? 'text-right' : 'text-left'}`}>
                    {isDepartmentMessage(message) ? 'You' : (message.senderId?.name || 'User')}
                  </div>
