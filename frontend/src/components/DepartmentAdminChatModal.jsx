...
const DepartmentAdminChatModal = ({ isOpen, onClose }) => {
    ...
    if (isOpen) {
      const initializeChat = async () => {
        ...
        try {
          ...
          if (!token) throw new Error('You are not logged in as department. Please login again.');
          
          // Create or get existing chat with city admin
          const chatResponse = await fetch('http://localhost:5000/chat/admin-dept/create-with-city', {
            method: 'POST',
            headers: {
          ...
          if (!chatResponse.ok) {
            ...
            try {
              ...
              console.error('Dept-admin chat creation failed:', errorData);
            } catch {
              /* ignore parse error */
            }
          ...
          console.error('Error initializing dept-admin chat:', error);
  ...
  const loadMessages = async (chatId) => {
    try {
      ...
      console.error('Error loading dept-admin messages:', error);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      console.error('Error sending dept-admin message:', error);
  ...
  const isDepartmentMessage = (message) => {
    return message.senderModel === 'Department';
  };
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600">
              {chat?.adminId?.name ? `${chat.adminId.name} - ${chat.adminId.assignedCity}, ${chat.adminId.assignedState}` : 'Loading admin info...'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {loading ? (
            ...
            <div className="text-center text-gray-500 mt-8">
              <p>No messages yet. Start the conversation with your city admin!</p>
            </div>
          ) : (
            messages.map((message) => (
              <div
                key={message._id}
                className={`flex ${isDepartmentMessage(message) ? 'justify-end' : 'justify-start'}`}
              >
                <div className="max-w-xs lg:max-w-md">
                  {/* Sender name */}
                  <div className={`text-xs text-gray-500 mb-1 ${isDepartmentMessage(message) ? 'text-right' : 'text-left'}`}>
                    {isDepartmentMessage(message) ? 'You (Department)' : (message.senderId?.name || 'City Admin')}
                  </div>
                  
                  {/* Message bubble */}
                  <div
                    className={`px-4 py-2 rounded-lg ${
                      isDepartmentMessage(message)
                  ...
                  {/* Timestamp */}
                  <div className={`text-xs text-gray-400 mt-1 ${isDepartmentMessage(message) ? 'text-right' : 'text-left'}`}>
        ...
        <form onSubmit={sendMessage} className="p-4 border-t border-gray-200">
          <div className="flex items-end space-x-2">
            ...
            <input
              ...
              placeholder="Type a message to city admin..."
              disabled={sendingMessage}
...
export default DepartmentAdminChatModal;